...
```mermaid
flowchart TD
  A["HO-3 policy"] --> B{"HO 04 54 attached"}
  B -- "No" --> C["Base earth-movement exclusion"]
  B -- "2009-04" --> D["2009-04 contract terms"]
  B -- "2021-12" --> E["2021-12 contract terms"]
  D --> F{"California policy"}
  E --> F
  F -- "No" --> G["Apply the attached edition"]
  F -- "Yes" --> H["Read HO 01 04 and the applicable CDI offer rules"]
  H --> I{"Offer accepted and coverage issued"}
  I -- "No" --> J["Offer and response are compliance records"]
  I -- "Yes" --> K["Apply the issued earthquake coverage"]
```
