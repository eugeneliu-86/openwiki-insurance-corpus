...
```mermaid
flowchart TD
    start["Identify the policy edition"] --> docs["Read declarations and attached endorsements"]
    docs --> property["Classify the claim under B C D E or F"]
    property --> peril["Test covered property and the edition specific peril"]
    peril --> exclusions["Apply exclusions and tenant property boundaries"]
    exclusions --> override["Apply an attached line appropriate endorsement"]
    override --> duties["Check notice protection records proof and cooperation"]
    duties --> valuation["Apply settlement valuation limits and deductible"]
    valuation --> dispute["Use appraisal only for amount of loss"]
    dispute --> payment["Apply the edition specific payment rule"]
```
