...
```mermaid
flowchart TD
    A["Notice and policy status"] --> B["Identify source and timeline"]
    B --> C{"Is cause supported"}
    C --> D["Separate covered damage from excluded conditions"]
    C --> E["Preserve evidence and refer unresolved cause"]
    D --> F["Scope property and necessary mitigation"]
    E --> F
    F --> G["Value supported covered damage"]
    G --> H["Apply terms limits deductible and authority"]
    H --> I["Pay explain or escalate"]
```
